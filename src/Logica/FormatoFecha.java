...
public class FormatoFecha {
    ...
    public static Date fecha(int dia, int mes, int anio) {
        ...
        if (anio < 1900 || anio > 2200) {
            throw new IllegalArgumentException(Color.rojo(Color.negrita(" Año no valido")));
        }

        // Comprobar que el mes este entre 1 y 12
        if (mes < 1 || mes > 12) {
            throw new IllegalArgumentException(Color.rojo(Color.negrita(" Mes no valido.")));

        } else { // Si es valido, que compruebe el dia

            // Comprobar que el dia este entre 1 y 31
            if (dia > 31 || dia < 1)
                throw new IllegalArgumentException(Color.rojo(Color.negrita(" Dia no valido")));
            ...
            switch (mes) { // Excepciones en meses
                ...
                case 11:
                    if (dia > 30) {
                        throw new IllegalArgumentException(
                                Color.rojo(Color.negrita(" Dia " + dia + " no valido para el mes " + mes)));
                    }
                    break;
                ...
                case 2:
                    ...
                    if ((anio % 4 == 0 && anio % 100 != 0) || (anio % 400 == 0)) {
                        ...
                        System.out.println();
                        throw new IllegalArgumentException(
                                Color.rojo(Color.negrita(" Dia " + dia + " no valido para febrero en año bisiesto ")));
                    } else {
                        if (dia > 28) { // Si no es bisiesto, comprobar que no sea mayor a 28
                            throw new IllegalArgumentException(
                                    Color.rojo(Color.negrita(" Dia " + dia + " no valido para febrero")));
                        }
                    }
                    break;
            }
